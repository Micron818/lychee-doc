...
- **已被 AP 占用的数量不能退。** 提示区分：对方是 DRAFT 则改/删发票行；已过账则先冲发票。i18n，不要静默截断

贷项二期：过账链路预留接口，见 §7。V1 不实现、不调用。
...
6. 非赠品：冲回 GR/IR（预留贷项 hook，V1 空实现，见 §7）
...
过账在写 GR/IR 之前调用扩展点（空实现即可，必须留类型，禁止只写注释）：

```text
RemotePurchaseReturnCreditMemoService
  assertReturnableAgainstInvoice(receiptItemId, returnTxnQty)
      // V1：returnTxnQty ≤ uninvoicedTxn，否则 ValidationException
      // 二期：已开票部分改为要求已存在过账贷项，或本方法内生成贷项

  afterPosted(purchaseReturnId)
      // V1：no-op
      // 二期：按已开票退货数量生成/核销 AP 贷项
```

WM 过账只依赖此接口，不要在 WM 里写 AP 实体。二期改 FI 实现，不改数量账公式以外的 WM 主流程。
...
| 贷项预留 | `RemotePurchaseReturnCreditMemoService`（新，V1 只做未开票断言） |
