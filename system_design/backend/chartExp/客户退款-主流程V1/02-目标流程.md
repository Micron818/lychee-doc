...
实收现金上限在 **贷项过账** 校验（见 [应收贷项](../应收贷项/02-目标流程.md) §3.5），本波只消费已拆好的 `refund_remaining`：
...
cashReceived = Σ(
  ...
  AND allocated_amount          // 不含 discount；不含 PREPAYMENT
)
...
- 创建/更新/过账校验：内部银行同公司、同币别、有效，银行 GL 属于该公司；对方银行若填则必须是该客户在 `partner_bank_accounts` 下的有效户（现网客户/供应商共用此表，对标 `assertSupplierRefundBankAccounts`）。**仅 CUSTOMER_REFUND** 强制，不扩大到 STANDARD
...
原收款：目标 AR 存在非 VOIDED 且 `refundable > 0` 的贷项时，禁止 `removeAllocation` 该 AR 行、禁止 VOID 含该行的原收款。

锁序（全序，防止与贷项 POST 的 CM→AR 死锁；退款 VOID 即使不改 AR 也要按同一顺序拿锁，禁止实施时图省事删掉原 AR 锁）：

```text
退款 POST / VOID：Payment → CreditMemo → 原 AR
贷项 POST / VOID：CreditMemo → 原 AR
收款 STANDARD：Payment → 原 AR
```

关帐：客户退款复用现网 `payments` 未过账检查（`payment_date` + `DRAFT/PENDING_APPROVAL/APPROVED`），本波不加新关帐查询。
...
可退款查询消费应收贷项本波已交付的 `POST /api/v1/fi/ar-credit-memos/refundable/page`；本波不再重做该 API。
