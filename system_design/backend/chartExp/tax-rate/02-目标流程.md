...
5. 上列 1–4 把 company_id 换成 NULL（该国租户默认）
6. 仍无 → ValidationException，禁止写行
```

同层禁止两行（唯一约束含 `country_code`，因此 VN / CN 可各写一套租户默认）。不要「找不到就 0%」。
...
| `/fi/tax-determinations` | 单表；国家、公司（可空=该国租户默认）、方向、两个分类（可空=通配）、税码 |
...
14. **判定矩阵国家** — 行上存 `country_code`，租户默认按国家各写一套。  
