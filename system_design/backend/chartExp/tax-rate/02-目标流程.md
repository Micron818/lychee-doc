...
- 物料分类：`materials.tax_class_id`（必填）。
- 无物料的 AP 杂项行：只用往来分类 + 物料分类通配（判定行 `material_tax_class_id IS NULL`）。
- `documentDate`：SO/PO/委外用 `order_date`；发票用 `invoice_date`。用于取税率档，**不是**判定矩阵的键。

### 3.2 匹配顺序（先公司后租户，先具体后通配）

同一 `tenant` 下只取 `is_active = true` 且税码国家 = `companies.country_code`、税码方向兼容（`INPUT`/`OUTPUT`/`BOTH` 与本次方向匹配）的行：
...
5. 上列 1–4 把 company_id 换成 NULL（租户默认）
6. 仍无 → ValidationException，禁止写行
```

同层禁止两行（唯一约束）。不要「找不到就 0%」。
...
| DRAFT 用户改税码 | 按单据日重取税率档并重算 |
| 已审核业务单改税 | **禁止** |
| GR / 发货 从上游建行 | **复制** `tax_code_id` + `tax_rate`，不判定 |
| AP/AR 从 GR/发货/订单建行 | **复制**；允许改税码（重取税率）或改 `tax_amount` 对齐票面 |
| AP/AR 无来源杂项行 | **判定**（物料分类通配） |
...
- 改 `line_amount`：若用户未手改税额，按公式重算税额；若已手改税额，只重算 `total`（实现用「税额与公式差额 > 0.00」视为手改，或行上 `tax_amount_overridden` 布尔；**采用显式布尔**，见 03）。
...
判定矩阵只允许指向 `country_code` 与公司国家相同的税码。
...
| `/fi/tax-determinations` | 单表；公司、方向、两个分类（可空=通配）、税码 |
