...
MRP 结果列表的 PURCHASE 按钮 **跳转采购建议工作台**（可带工厂）；**不要**在结果页做无供应商的一键提交。**不再提供「转采购申请」。**
...
计划员从 MRP 结果页进入：跳转工作台。工作台仍只接受各厂最新 Run；打开的若已不是最新，提示「请使用最新 Run / 采购工作台」。
...
MRP 结果页 PURCHASE 过滤下的按钮改为跳转：

```text
/scm/purchase-proposals?factoryId={result.factoryId}
```
