...
| `/{id}/submit` | POST | D | ✅ | ❌ | ❌ | ❌ | ❌ | PA | |
| `/{id}/approve` | POST | PA | ❌ | ✅ | ❌ | ❌ | ❌ | A | 写 `approvedAt` / `approvedBy` |
| `/{id}/post` | POST | A | ❌ | ❌ | ✅ | ❌ | ❌ | P | 银行账户 GL；按头 `amount` 生成 **AUTO 凭证**（不要求有行）；若已有拟核销行则应用并反写发票；保留 `unallocatedAmount` |
| `/{id}/void` | POST | 非 V | ✅ | ✅ | ✅ | ⚠️ | ❌ | V | P → **冲销银行凭证 + 回滚全部核销行 + 恢复发票收付金额** |
...
| API | Method | 前置 | 行为 |
|-----|--------|------|------|
| `/{id}/allocations` | POST | P，`unallocatedAmount` 足够 | 新增核销行并立即反写发票；`discount`/`GL_ACCOUNT` 产调整凭证并写回 `payment_lines.journal_entry_id` |
| `/{id}/allocations/{lineId}` | DELETE | P | 回滚该行发票影响，重算 `unallocated`，按行 `journal_entry_id` 冲调整凭证 |

### 5.4 收付款行引用发票

| 场景 | 校验 |
|------|------|
| `allocationType = INVOICE` + `apInvoiceId` | AP 必须 `POSTED`；DISBURSEMENT；partner/company/currency 一致；≤ remaining |
| `allocationType = INVOICE` + `arInvoiceId` | AR 必须 `POSTED`；RECEIPT；partner/company/currency 一致；≤ remaining |
| `allocationType = INVOICE` 且两者皆无 | 拒绝 |
| `allocationType = GL_ACCOUNT` | 必须 `glAccountId` |
| 同单重复核销同一发票 | 拒绝 |
| Σ 核销额 > `amount` | 拒绝 |
...
|------|----------|
| 编辑 / 删行 / 删单 | `DRAFT` |
| 添加拟核销行（items） | `DRAFT`；可选发票仅 **POSTED** |
| 核销（allocations） | `POSTED` 且 `unallocatedAmount > 0` |
| 提交 / 审批 / 过账 | D → PA → A → P |
| 作废 | `!== VOIDED` |
...
| Payment POSTED | 阻塞发票 void | 应先 void Payment |
| Payment POSTED void | 冲销凭证 + 恢复发票额度 | Payment「作废」可用 |
| AUTO 凭证 | 不可手工 edit/void | Journal 仅「查看 + 跳转来源」 |
| 会计期间已关 | post / reverse 等校验失败 | 过账/冲销需判断期间 |

正确作废顺序（发票已收付款）：

```
1. void Payment（冲销付款凭证，恢复发票 remaining / 业务状态）
2. void Invoice（冲销发票凭证）
```
...
*文档版本：与后端 `lychee-erp-fi` 模块当前实现同步（含 Payment 先资金后核销、`/allocations` API、AR/AP 核销对称校验）。*
