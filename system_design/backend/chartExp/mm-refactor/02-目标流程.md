...
流水按「该款+该色」已占用+本次勾选，从小到大；已占用号不重排
```
...
1. **尺码组**：组内 `product_size_id` + `sequence`（建议 10,20,30，从小到大）。**组上不存 SKU 流水。** 作用：限制可选码、决定「谁比谁小」。
2. **款号**绑定一个尺码组（生成必填）。
3. **本款颜色**
   - **≥1 行**：区分颜色。每行 `color_id` + `sku_code`（`A`–`Z`）+ `sequence`。SKU 含该字母。
   - **0 行**：不区分颜色。SKU **无字母**；生成格子 `color_id` 为空。
4. **款×色尺码流水**（`product_model_size_codes`）：`(款号, 颜色可空, 尺码) → 01–99`。生成时按 §3.1 分配并落库。已有物料占用后禁止改号。

新增本款色：默认下一个空闲字母。该款已有物料后，禁止在「0 行」与「≥1 行」之间切换。
...
单笔保存：已绑组则尺码必须在组内。本款 ≥1 色则 `color_id` 必须在本款；本款 0 色则成品 `color_id` 必须空。原料三维度仍可空。

### 2.4 Wave B：报表与复用

组内 `sequence` 改为 FO/SO 打印列序的权威（有组时）。`product_sizes.sequence` 仅无组时兜底。
...
- **分配**：作用域内「已有流水 ∪ 本次勾选」按尺码组 `sequence` 从小到大：已有号保持；新码从该作用域 `max+1` 起编（尚无任何号则从 `01`）。
...
- 预览只读，不写库。
- 生成单事务：全部插入成功或全部回滚。
- `EXISTS` 不插入。选中的格若预览后被人抢先插入：唯一索引冲突 → 整批失败，提示重预览。
- 禁止前端循环 `POST /materials` 冒充生成器。

### 3.6 不改的行为

- 删除物料、BOM 引用拦截、工厂物料页、图片管理页：现网不变。
...
现网 `POST /api/v1/mm/materials` **保留**。生成内部走同一套校验 + 唯一键，不另写一套 Entity。
...
- 不分色：单行矩阵，格上 `12345-01`。
- EXISTS 灰显。勾选格不得含 CONFLICT / OVERFLOW / MISSING_TOKEN。

### 5.2 单笔表单

§2.3。QuickCreate 仍建租户色卡/尺码。

### 5.3 Wave A 主数据页

- 尺码组：头 + 组内行（显示名、`sequence`）。**无 sku_code 列。**
- 款号：`sizeGroupId`；本款颜色（0 行=不分色；有行则字母 A–Z）。
- 菜单：「尺码组」。
