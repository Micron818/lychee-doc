...
* **数据约束**：
  ...
  - **变体唯一性与排他性**：由领域策略服务 `assertVariantUnique`（业务查重，仅限变体分类）与 `materials` 物理表 `UNIQUE(tenant_id, code)`（高并发兜底）双重保障，彻底解耦 `materials` 物理表上对变体三维的硬性部分唯一索引。
* **作业入口**：
...
2. **分类变更约束（Category Switch）**：
   - 若用户修改物料分类，系统重新解析目标分类的有效策略：
     - 若新旧策略一致（如 `SEQUENTIAL` -> `SEQUENTIAL` 或 `MANUAL` -> `MANUAL`）：允许修改分类，但**不重新生成已分配的物料编码**，避免单据断链；
     - 若试图在 `FASHION_VARIANT`（鞋服变体）与非变体策略（`SEQUENTIAL` / `MANUAL`）之间跨策略变更分类：**后端直接抛出业务异常予以拦截（`validation.material.category.strategyMismatch`）**，因为变体物料编码、款式维度及尺码流水一旦生成落库已具备特定领域语义，不可随意篡改其主档性质；如确需更换业务属性，必须将旧物料标记为停用，并在新分类下建档。
3. **变体三维锁定（V1 规则继承）**：
