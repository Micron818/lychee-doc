...
* **编码公式**：
  ...
  - **Separator（分隔符）**：默认 `-`；
  - **DateSegment（可选日期）**：如 `YYYYMM`（`2609`）；
  - **Sequence（递增序列）**：支持固定位数（如 4~6 位），高位补零（如 `00042`）。
* **数据约束**：
  - 款号、颜色、尺码**不是编码的一部分**，表单默认隐藏或收折这些维度；
  - 保存时若前端未传入 `code`，由后端的 `DocumentNumberGeneratorService` / 序列分配器在持久化事务中原子递增分配，确保 100% 连续且并发安全。
...
sequenceDiagram
    ...
    alt 策略为 SEQUENTIAL
        MatAPI->>SeqGen: allocateCode(categoryId, tenantId)
        SeqGen->>DB: 原子自增获取下一个流水 (FOR UPDATE)
        DB-->>SeqGen: 返回流水号 00128
        SeqGen-->>MatAPI: 拼接完成 "FAB-2609-00128"
