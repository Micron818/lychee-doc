...
3. **编码前缀（`codePrefix`，仅 SEQUENTIAL）**：
   ...
   - 若未配置且处于独立号池模式（`is_seq_shared = false`）：沿树向上寻找首个显式配置的 `code_prefix`；若全链均无显式配置，**默认使用当前选中节点自身的分类代码 `code`**（例如针织面料 `KNIT` 独立发号，默认前缀即为 `KNIT`）。
...
flowchart TD
    ...
    SelfPool --> ResolveParamsSelf[解析前缀: 优先自身prefix -> 链上首个prefix -> target.code]
...
public EffectiveCodingPolicy resolvePolicy(Long categoryId, Long tenantId) {
    ...
    if (isShared) {
        // 向上寻找号池属主 (优先选择策略源，若策略源是自身则取其直接父级)
        if (strategySourceCat != null && !strategySourceCat.getId().equals(target.getId())) {
            poolOwner = strategySourceCat;
        } else {
            poolOwner = chain.get(1); // 锚定直接父分类
        }
    }
    ...
    if (StringUtils.hasText(target.getCodePrefix())) {
        ...
        // 独立号池向上继承首个显式 prefix，若均未配置则回退自身 code
        for (MaterialCategory cat : chain) {
            if (StringUtils.hasText(cat.getCodePrefix())) {
                resolvedPrefix = cat.getCodePrefix();
                break;
            }
        }
