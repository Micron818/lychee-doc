...
public EffectiveCodingPolicy resolvePolicy(Long categoryId, Long tenantId) {
    ...
    Long initialParentId = current != null ? current.getParentId() : null;
    ...
    while (current != null) {
        if (current.getCodeStrategy() != null) {
            return EffectiveCodingPolicy.builder()
                ...
                .codePrefix(StringUtils.hasText(current.getCodePrefix()) ? current.getCodePrefix() : current.getCode())
                .seqLength(current.getSeqLength() != null ? current.getSeqLength() : 5)
                .dateFormat(current.getDateFormat())
                .isSeqShared(Boolean.TRUE.equals(current.getIsSeqShared()))
                .build();
        }
...
## 6. 物理約束解耦（Decoupling Physical Constraints）

在 V1 實施中，`materials` 物理表上創建了無條件的部分唯一索引 `(tenant_id, product_model_id, color_id, product_size_id)`。
這在 V1 單一假設（所有使用款號的物料都必須全局款色碼唯一）下看似合理，但在 V2 多策略體系下產生了嚴重衝突：
- **OEM 客供料號衝突**：代工不同客戶的同一版型無法建立不同的自定義物料號；
- **跨表不可見**：資料庫索引條件無法穿透至分類表動態判斷 `code_strategy`。

因此，V2 的核心架構原則是：**「底層物理表 materials 僅保留 `UNIQUE(tenant_id, code)`，變體維度的業務唯一性上移至策略領域服務（`assertVariantUnique`），並由流水表 `product_model_size_codes` 的物理索引負責並發排他」**。這使 ERP 同時具備了鞋服的防撞能力與代工/離散製造的靈活適應力。
