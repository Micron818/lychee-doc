...
交货行新增 `returned_quantity`（**单据单位**，仅已过账客户退货累加），供审计 / 展示与交叉核对。**开票闸与可退未占用池用 `issued_quantity`**（过账已由 `issueCallback` 回减，等于实发净额），不要用 `txn − returned`。
...
otherDraftOnDI        = SUM(其他 DRAFT 客户退货行.txn)  // 原领料行.source_doc_item_id = 该交货行，排除本单
remainingUninvoiced   = delivery.issued_quantity − delivery.invoiced_quantity − otherDraftOnDI
returnableTxn         = min(unreturnedTxn, remainingUninvoiced)
```

`issued` 已净掉 POSTED 退货，**不要**再减 `postedReturnedOnDI`。部分出货时 `issued < txn`，用 `txn − invoiced − returned` 会把未发量算进可退 / 可开票。
...
- **基本单位清尾**：用户改数量时 txn→base 走现有单位服务。当本行是「最后一笔把该领料行未退单据量退完」（`transaction_quantity = unreturnedTxn`）时，`base_quantity` **直接取** `unreturnedBase`，禁止再换算后留下或溢出零头

分层：WM 计算器负责完整公式（含其他草稿退货）。FI `assertReturnableAgainstInvoice` **只**断言开票占用维：`returnTxnQty ≤ issued − invoiced`（`issued` 已净 POSTED 退货）。草稿退货池不要让 FI 再查 WM 实体。
...
| 字段 | 规则 |
|------|------|
| `warehouse_id` | 从原领料行复制，不可改。非库存物料（`MaterialInventoriabilityResolver.isInventoried = false`）保持不过库存 |
...
- **金额清尾**：该领料行原出库成本金额 − 已过账退货已冲回金额 = 剩余；当本行是「最后一笔把未退基本量退完」时，COGS 冲回用**剩余金额**，避免分次取整留分钱
- **非库存物料**：不过 `stock_on_hand`、不写流水账；该行不进 COGS 请求（对标原领料 `unitCost == null` 则跳过）。已退数量与交货/SO 仍回写
...
`return_date` = 凭证 `postDate`。财务期间关闭时过账失败。数量记账无凭证时与领料一样不拦库存期间；本波不新做库存关帐（锁定，见 `04` §2）。

库存时点：过账用过账时刻写 `stock_transactions.transaction_date`；冲销用冲销时刻。冲销凭证走现有 `reverseEntry`，不换日。

客户退货 **不** 冲收入 / 销项税。收入侧只在 [应收贷项](../应收贷项/README.md)。

### 3.6 过账

同一事务（锁、计算器、断言的细分见 `03` §7）：

1. 锁原领料行、关联交货行，重算可退，超则失败
2. 库存物料：`applyCustomerReturn`（`CUSTOMER_RETURN`，入库）
3. 领料行 `returned_quantity += base_quantity`
4. `issueCallback(−transaction_quantity)`：回减 `issued` / SO 已交，并按 `issued` 刷新 `invoice_status`（正负向都刷，见 §3.9）
5. `updateReturnedQuantity(+transaction_quantity)`：只写交货行已退审计列，**不**刷新 `invoice_status`
6. 非数量记账：冲回 COGS（`afterPosted` 保持 no-op；贷项在 FI 独立过账）
7. 表头 `approved_by` / `approved_at` 记过账人（对标采购退货 / 退料）
8. 状态 → `POSTED`

不删 `DELIVERY → STOCK_ISSUE` 溯源。靠 `original_issue_item_id`。

`issueCallback` 本波必须补负向状态，并在 **每次 `issued` 变化后**（销售出库过账 `+qty`、冲销出库、退货 `−qty`、冲销退货 `+qty`）按 `resolveItemStatus(issued, invoiced)` 刷新该行与表头 `invoice_status`。现网只在 `updateInvoicedQuantity` 刷状态；分母改成 `issued` 后，正向再出库不刷会把已开满的部分出货钉死在 `FULLY_INVOICED`。
...
| SO 表头 | `CLOSED` / `CANCELLED`：不改状态。`DELIVERED` 且不再全交 → `CONFIRMED`（未交量重新进入可交 / MRP 汇总）。表头仍为 `CONFIRMED` 时**保留**现网：全行 `DELIVERED` → `DELIVERED`；全行 `CLOSED` 或 `DELIVERED` → `CLOSED` |

退货路径不会把行改成 `CLOSED`，因此负向回退不会触发自动关单。`CLOSED` 的 SO 允许退货（货是真退），但不得无声复活进 MRP。
...
3. `issueCallback(+transaction_quantity)`。回加已发视同再出库：交货行不得超过 `transaction_quantity`（现网 `issued_quantity.exceeds`）。将超则拒绝，提示先冲后补的出库。`invoice_status` 在此回调内按新 `issued` 刷新
4. `updateReturnedQuantity(−transaction_quantity)`：只减审计列（不得负）
...
交货取消：现网 `cancelDelivery` 只允许 `RELEASED` 且 `issued = 0`、无草稿领料。已过账销售出库后取消已被拦住，**不必**再加客户退货闸。

### 3.9 从交货开 AR

可开票数量（实发净额，不要用 `txn − returned`）：

```text
billableTxn   = issued_quantity − invoiced_quantity
newInvoiced   ≤ issued_quantity
billableTotal = issued_quantity
```

`delivery_items.returned_quantity` 只作审计 / 展示，**不作**开票闸分母。`invoice_status` 的刷新放在 `issueCallback` 内（正负向都刷）；`updateReturnedQuantity` 只写审计列。部分出货与部分退货共用 `issued` 口径。

- 草稿退货不扣可开票（过账才改 `issued`）；草稿已占用可退，避免超开+超退并发时由 §3.3 再拒
- **强制闸门**在 `updateInvoicedQuantity`：`newInvoiced ≤ issued`（替换现网只比 `transaction_quantity`）；改占用后仍按 `issued` 刷新状态
- `invoice_status`：`resolveItemStatus(issued, invoiced)`。**语义变了**：`FULLY_INVOICED` 表示「已出净额开完」，不是「整行 txn 开完」。`RELEASED` 且只出一部分、已开满已出量的交货可以是 `FULLY_INVOICED`；再出库后 `issueCallback` 必须把状态刷回 `PARTIALLY_INVOICED`。前端 / 报表用 `issued`、`invoiced`、`txn` 三列展示，不要只看状态当成「整单结束」
- 全退且从未开票：`issued = 0` 且 `invoiced = 0` → 仍为 `UNINVOICED`；`getDeliveryForBilling` 无剩余行时用明确 i18n（不要当成 `already_invoiced` / `not_delivered`）
- **放开表头状态闸**：现网要求表头 `DELIVERED`（隐含 `issued = txn`）。放开后必须改用 `issued`，否则 `RELEASED` 且只出了一部分的交货能对未发量开票。改为：非 `CANCELLED` / `DRAFT`，且存在 `billableQty > 0` 的行即可开票。`FULLY_INVOICED` 仍拒。从交货开 AR 的其它入口同一口径
...
V1 硬规则：已占用数量不可退（含 AR 草稿）。`RemoteCustomerReturnCreditMemoService` **待实现**（对标 `RemotePurchaseReturnCreditMemoServiceImpl`）：`assertReturnableAgainstInvoice` 只看交货行开票占用；`afterPosted` / `afterReversed` 保持 no-op。禁止用 `// TODO` 代替类型。
...
| 回减 / 加回交货已发、SO 已交、刷新 `invoice_status` | 现有 `RemoteDeliveryService.issueCallback`（正负向都刷；保留 CONFIRMED→CLOSED） |
| 交货已退审计列 / 可开票闸 | `updateReturnedQuantity`（只写列）+ `updateInvoicedQuantity` / `getDeliveryForBilling`（上限 `issued`，放开 DELIVERED 闸） |
| 交货行占用只读 | `RemoteDeliveryService.findItemQuantity`（给 FI 断言） |
| COGS 冲回 | `RemoteCustomerReturnFinanceService`（新，对标领料 finance） |
| 未开票断言 | `RemoteCustomerReturnCreditMemoService`（FI 实现；只看开票占用） |
| 已过账退货查询 | `RemoteCustomerReturnService.existsPostedByDeliveryItemIds`（WM 实现；贷项波可先 stub `false`） |
| 贷项 | FI 独立子域，见 [应收贷项](../应收贷项/README.md)。WM 不写 AR / 贷项实体 |
