...
> 原物料採購流程改造見 [chartExp/Purchase](../chartExp/Purchase/README.md)。
...
```text
原物料:  MrpResult ──工作台──► Purchase Order ──► GR (WM) ──► AP
手工請購:  PR（MANUAL）──審核──► 匯入 PO ──► GR (WM) ──► AP
委外:     Outsource Order ──► 發料 / 收貨 (WM)
```

請購單 **只服務手工請購**（耗材、備件、非 MRP）。系統未上線前，MRP 路徑停止再寫 `source_type = MRP` 的 PR。

## 2. 實體關係圖 (ERD) 概念

*   **供應商**: `suppliers`。
*   **物料來源清單（規劃）**: `material_suppliers`（廠 + 料 + 供應商），供工作台預填供應商與 PO 帶價。
*   **請購 (PR)**: `purchase_requisitions` / `purchase_requisition_items`。內部申請；目標僅 `source_type = MANUAL`。
...
|------|--------|----------------------|
| 原物料轉 PO（目標） | `MRP_RESULT` | `PURCHASE_ORDER` |
| 手工 PR → PO（現行） | `PURCHASE_REQUISITION` | `PURCHASE_ORDER` |
| 過渡期 MRP → PR（將淘汰） | `MRP_RESULT` | `PURCHASE_REQUISITION` |
...
### 3.2 物料供應商 (`material_suppliers`) — 規劃新增
...
內部請購。目標路徑僅手工建立；MRP 不再落此表。

*   **表頭關鍵欄位**:
    *   `requester_id` / `department_id` / `factory_id`。
    *   `source_type`: `MANUAL`（目標日常）；現行枚舉仍含 `MRP`，停止新寫入。
    *   `mrp_run_id`: 僅歷史 MRP PR；手工為 NULL。主檔不加 `source_mrp_result_id`。
...
*   **表頭關鍵欄位**:
    ...
    *   `source_type`（**規劃新增**）: `MRP` | `MANUAL`。工作台生成 = `MRP`；畫面新增 = `MANUAL`。  
        **不加** `mrp_run_id`（一張 PO 可含多個 Run 的建議；Run 經 pegging 反查）。
    *   `status`: `DRAFT` / `APPROVED` / `PARTIAL` / `COMPLETED` / `CLOSED`。
*   **明細關鍵欄位**:
...
已審核 / 部分收貨的 PO 開量進入 MRP 預計入庫；**DRAFT 不計入**（與現行 DRAFT PR 相同）。
...
| **material_suppliers** | （規劃，尚未建檔） | 物料來源清單；DDL 見 Purchase/03 |
| **purchase_requisitions** | `sql/schema_tables/SCM/purchase_requisitions.sql` | 請購表頭 |
| **purchase_requisition_items** | `sql/schema_tables/SCM/purchase_requisition_items.sql` | 請購明細 |
| **purchase_orders** | `sql/schema_tables/SCM/purchase_orders.sql` | 採購表頭（規劃加 `source_type`） |
