...
COMMENT ON COLUMN lychee_erp.stock_transactions.transaction_type
	IS 'GOODS_RECEIPT,PRODUCTION_RECEIPT,PURCHASE_RETURN,DELIVERY,CUSTOMER_RETURN,STOCK_ISSUE,STOCK_ISSUE_RETURN,ADJUSTMENT,STOCK_TRANSFER,BACKFLUSH。冲销沿用原过账类型（进出方向对调）；PURCHASE_RETURN 仅用于采购退货单；STOCK_ISSUE_RETURN 仅用于领料退料单'
;
