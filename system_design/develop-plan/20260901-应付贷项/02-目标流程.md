...
>
> 本文件是已落地的应付贷项口径：贷项可超过未付剩余并拆出待退款；真实现金退款见 [供应商退款-主流程V1](../供应商退款-主流程V1/README.md)。完整目标见 [供应商退款](../供应商退款/README.md)。
...
应付贷项    ─► 部分数量/金额；原票保留 POSTED；applied 冲未付，超出部分记 refundable
```
...
- 表头 `total_amount ≤ 原票尚未贷回总额`（`total − credited`，过账时锁原票）。允许超过 `remaining`，超出部分形成待退款。
- 已付部分可贷：贷项过账拆 `applied`（冲未付）与 `refundable`（待退款）；真实退款见 [供应商退款-主流程V1](../供应商退款-主流程V1/02-目标流程.md)，不要作废原付款。
- **草稿互斥不对称**：数量在草稿即互斥；金额只在过账用尚未贷回总额与现金上限卡。两张草稿金额可以都合法，第二张过账失败。前端可贷金额不要按「草稿已预占 remaining」展示
...
```text
remaining = total_amount − paid_amount − applied_credit_amount
```

- 贷项过账：`credited_amount += 本单 total`，`applied_credit_amount += applied`；重算 `remaining` 与 `payment_status`
- 贷项 VOID：回滚 `credited_amount` 与 `applied_credit_amount`（不得负）
- 付款 `applyAp` / `restoreAp` **必须**改用上式，禁止再写 `remaining = total − paid`，也禁止再用 `credited` 进 remaining
...
3. 重算可贷数量；超则失败
4. `total_amount ≤ 原票尚未贷回总额`（`total − credited`）
5. 成卡门（§3.8）
6. 写凭证
7. `credited_amount` / `applied_credit_amount` / `remaining` / `payment_status` / 贷项拆分字段
...
4. 回减 `credited_amount` 与 `applied_credit_amount`，重算原票 remaining / payment_status。有有效供应商退款则拒绝。
...
| 过账付款 | 不直接拦贷项；待退款不得超过剩余实付现金。有 `refundable>0` 的未作废贷项时禁止撤原付款 |
...
- 未分配贷项（过账后必须核完本单金额）
- 一张贷项核多张 AP
- 已付部分退款 / 供应商退款用途（下一波见 [供应商退款-主流程V1](../供应商退款-主流程V1/README.md)）
- 贷项付款排程
- 成卡后原值调整
- 从采购退货自动生成贷项
