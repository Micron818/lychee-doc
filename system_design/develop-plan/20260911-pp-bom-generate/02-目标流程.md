...
款号 ProductModel
  │
  ├─ 已存在的自制色 × 码 SKU（productSizeId ∧ isManufactured；不看 is_fashion_variant）
  │
...
3. 步骤 2：勾选本款颜色、尺码（数据源 `GET /mm/product-models/{id}/variant-setup`）。不分色则无颜色多选。只生成**已存在**的自制色 × 码 SKU（不看 `is_fashion_variant` / 分类编码策略）；缺料格子预览为 `MISSING_SKU`，不在本波建料。同格多颗合格父件为 `CONFLICT`。
4. 步骤 3：`EditableProTable` 维护模板行：项次、匹配模式、种子物料、默认用量、可选按码用量（`sizeQuantities`）、损耗、倒扣、备注、行状态。至少一行。吊牌等全码同量只填默认用量；面料等按 **步骤 2 已勾尺码** 展开各码绝对用量（不要把尺码组未勾档位也铺成列）。
5. 步骤 4：调 `preview`。矩阵展示每格状态；展开或侧栏可看该格解析后的子件、**该格用量**与 **基本单位**。只勾选 `NEW` 格提交（`EXISTS` 跳过，坏格禁止勾选）。
...
```text
productModelId     = 请求款号
productSizeId      ≠ null          // 完整色 × 码格
materialType.isManufactured = true
activeStatus       = ACTIVE
```

**不看** `isFashionVariant`，**不读**分类 `code_strategy`。手建料只要挂了本款色码且自制 ACTIVE，即可作为父件。  
区分色：`colorId` 必须落在本款颜色池。不分色：`colorId` 必须空，请求 `colorIds` 必须空。  
同格合格父件 > 1 → 该格 `CONFLICT`（不静默挑选，不用变体标记决胜）。  
无色码矩阵的作业（无尺码组）仍走单笔，不进本向导。
...
```text
1. 无完整色 × 码 SKU            → MISSING_SKU
2. SKU 存在但 0 颗合格          → INELIGIBLE
     （非自制 / 非 ACTIVE / 色模式与本款不符）
3. 同格合格父件 > 1             → CONFLICT
4. 已存在 (productMaterialId, version) 的 BOM
                                 → EXISTS（带 existingBomId / bomStatus）
5. 任一模板行非 RESOLVED        → 格状态 = 最严重行态
...
- preview：矩阵列 + 行 + 扁平 `cells`（含 `items[]` 解析明细）。行列形状对标物料预览，**类型在 PP 自建**，禁止 import MM DTO。
...
容器用 **`Drawer width="90%"`**（步骤 3 有动态尺码列，70% Modal 不够）；不要改成物料那套 `Modal width="70%"`。
...
| 3 模板行 | EditableProTable + 按码用量列（仅步骤 2 已勾尺码） | 0 行；`COLOR_MATCH` 种子带尺码；非 FIXED 种子无款号；按码表重码 / 组外码 / ≤0 |
...
按码用量：步骤 3 只为 **步骤 2 已勾** 的 `productSizeIds` 展开尺码列（`sequence, productSizeId` 序），单元格绑定 `sizeQuantities[productSizeId]`。空单元格 = 回退默认 `quantity`，不要提交 `0`。回步骤 2 增勾尺码时新列为空（回退默认）。  
可选「按 % 填」用行内 **Popover / 小弹层**（基准码 + 各码 %），确定后折成绝对量写回这些列；不要在表格里再铺一套百分比列。提交 payload **只有** `quantity` + `sizeQuantities`。后端仍按本款尺码组校验组外码。
...
默认勾选全部 `NEW`。点格看 `items[]`：种子编码/名称 → 解析编码/名称、模式、行态、**该格用量 + 基本单位**（回退默认时标注）。
...
| 款号色码池、变体唯一查找 | PP 调 basis：`ProductModelRepository` / `ProductModelColorRepository` / `ProductSizeGroupItemRepository` / `MaterialRepository`（现网已有 fetch 方法）。**不要**调 MM `variant-setup` 或任何 MM Service |
| `GET .../variant-setup` | **不改**，**仅前端**直调 MM |
...
- 自动批准、生成后重算 LLC
- 运行时款号 BOM 继承
- 无色 × 码占用的父件批量（无尺码组或不挂码；编码策略本身不是门槛）
- Excel / EDI 导入 BOM
