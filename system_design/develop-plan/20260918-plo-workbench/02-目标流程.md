...
待转组件 `key={factoryId ?? 'all'}`：`form.initialValues` 只在挂载生效，换厂必须重挂载。
...
`rowKey="mrpResultId"`。  
默认排序：`requiredDate` `ascend`（越早越前）。列开 `sorter: true`。  
...
行内只留 `buildActions({ onEdit, onPegging })`：**编辑**（双击 / 操作列）、**Pegging**。CONVERTED 仍不可编、checkbox 禁用。  
**删除** `PlannedOrderMoreActionsButton`（`buildActions` 已有标准 Pegging 图标）。
...
删除非法时：`pages.common.bulkDeleteDraftOnly` 不套用（PLO 无 DRAFT）；有选且不可删时用 `pages.pp.plannedOrders.bulkDeleteConvertedForbidden`。  
`BatchDeleteButton` 的 `tooltipId` **空选不要传** forbidden 文案（组件会优先用传入的 `tooltipId`，空选会倒挂成「不能删已转单」）。对齐销售订单：
...
- 全部成功才提交；返回 `List<PlannedOrderResponse>`，**只保证 `id` / `orderNo`**。`saveAll` 后关联未 hydrate，不要再 fetch 拼物料名
...
- `mrp`：新组件 `components/MrpProductionWorkbench.tsx`（不要复用采购工作台；无供应商/单价/部分转）。调用处必须 `key={factoryId ?? 'all'}`，否则换厂时搜索初值不刷新
...
**删除** `PlannedOrderMoreActionsButton.tsx`。操作列直接：

```text
buildActions({
  onEdit,
  onPegging: handlePegging,
  isEditable: (record) => record.orderStatus !== PlannedOrderStatus.CONVERTED,
})
```
