...
| `/{id}/submit` | POST | D | ✅ | ❌ | ❌ | ❌ | ❌ | PA | |
| `/{id}/approve` | POST | PA | ❌ | ✅ | ❌ | ❌ | ❌ | A | 写 `approvedAt` / `approvedBy` |
| `/{id}/post` | POST | A | ❌ | ❌ | ✅ | ❌ | ❌ | P | 银行账户 GL；按头 `amount` 生成 **AUTO 凭证**。STANDARD 不要求有行，可保留 `unallocatedAmount`。**供应商退款**必须恰好一行 `AP_CREDIT_MEMO` 且 `allocated = amount`，过账后 `unallocated = 0`；凭证 `sourceDocType = SUPPLIER_REFUND`、模块 `AP`。**客户退款**必须恰好一行 `AR_CREDIT_MEMO` 且 `allocated = amount`，过账后 `unallocated = 0`；凭证 `sourceDocType = CUSTOMER_REFUND`、模块 `AR` |
| `/{id}/void` | POST | 非 V | ✅ | ✅ | ✅ | ⚠️ | ❌ | V | P → **冲销银行凭证 + 回滚全部核销行**。STANDARD：目标 AP/AR 有 `refundable>0` 的未作废贷项则拒绝。退款：先回滚贷项核销与汇兑凭证，再冲银行凭证 |

CRUD DTO 带 `paymentPurpose`（`STANDARD` / `SUPPLIER_REFUND` / `CUSTOMER_REFUND`）。Payment page 可按 purpose 过滤。

| API | Method | 说明 |
|-----|--------|------|
| `GET /by-line/{lineId}` | GET | 按核销行定位收付款单（凭证跳转 `PAYMENT` / `PAYMENT_ALLOC` / `SUPPLIER_REFUND_ALLOC` / `CUSTOMER_REFUND_ALLOC`） |
...
供应商退款草稿 `/{id}/items` 只允许恰好一行 `AP_CREDIT_MEMO`。客户退款草稿只允许恰好一行 `AR_CREDIT_MEMO`。DRAFT 且无行时可改 `paymentPurpose`。

### 5.3 核销 API（仅 POSTED）

| API | Method | 前置 | 行为 |
|-----|--------|------|------|
| `/{id}/allocations` | POST | P，`unallocatedAmount` 足够 | STANDARD：新增核销行并立即反写发票；`discount`/`GL_ACCOUNT` 产调整凭证。**供应商/客户退款：拒绝**（过账时已全额核销） |
| `/{id}/allocations/{lineId}` | DELETE | P | STANDARD：回滚该行；目标 AP/AR 有 `refundable>0` 的未作废贷项则拒绝。**供应商/客户退款：拒绝**（只整单 VOID） |
...
| `allocationType = AR_CREDIT_MEMO` | 仅 `paymentPurpose = CUSTOMER_REFUND`；贷项 `POSTED` 且 `refundRemaining > 0`；同行 `allocated =` 头 `amount`；同公司/客户/币别；交叉货币拒绝 |
| STANDARD 使用 `AP_CREDIT_MEMO` / `AR_CREDIT_MEMO` | 拒绝 |
| 同单重复核销同一发票 / 同一贷项 | 拒绝 |
| Σ 核销额 > `amount` | 拒绝 |

Open Invoices：现有 AP/AR page + filter（`invoiceStatus=POSTED` 且 `remainingAmount>0`）。  
可退款贷项：`POST /api/v1/fi/ap-credit-memos/refundable/page`（固定 `POSTED && refundRemaining > 0`；权限 `payments:read` + `ap-credit-memos:read`）。客户侧：`POST /api/v1/fi/ar-credit-memos/refundable/page`（权限 `payments:read` + `ar-credit-memos:read`）。
...
| 核销（allocations） | `POSTED` 且 `unallocatedAmount > 0` 且 **非供应商/客户退款** |
| 提交 / 审批 / 过账 | D → PA → A → P |
| 作废 | `!== VOIDED` |
...
## 5c、应收贷项（`/api/v1/fi/ar-credit-memos`）

独立 FI 单据，一对一已过账应收发票。过账允许 `total > remaining`：拆 `applied`（冲未收）与 `refundable`（待退款）。`cashReceived` 仅 STANDARD RECEIPT + INVOICE 的 `allocated`。占用仅 POST/VOID 回减交货 `invoiced_quantity`。存在非 VOIDED 贷项时禁止 VOID 原 AR。原收款在目标 AR 仍有 `refundable > 0` 时禁止撤核销 / VOID。

| API | Method | 说明 |
|-----|--------|------|
| `POST /page` | POST | 列表 |
| `GET /{id}` | GET | 详情 |
| `POST /` | POST | 新建（必须带 `originalArInvoiceId`） |
| `PUT /{id}` | PUT | 改草稿 |
| `DELETE /bulk-delete` | DELETE | 删草稿 |
| `POST /from-ar-invoice` | POST | 从已过账 AR 生成草稿并导入可贷行 |
| `PUT /action` | PUT | SUBMIT / APPROVE / UNAPPROVE / POST / VOID |
| `POST /{id}/items/page` | POST | 明细分页 |
| `POST /{id}/items` | POST | 加行 |
| `PUT /{id}/items/{itemId}` | PUT | 改行 |
| `DELETE /{id}/items/bulk-delete` | DELETE | 删行 |
| `POST /{id}/creditable-lines/page` | POST | 可贷原票行 |
| `POST /refundable/page` | POST | 可退款贷项分页；服务端固定 `POSTED && refundRemaining > 0`；权限仅 `/fi/ar-credit-memos:read` |

应收贷项响应含 `appliedAmount` / `refundableAmount` / `refundedAmount` / `refundRemainingAmount` / `refundStatus`。税务号字段为 `taxCreditNoteNo`。`POSTED && refundRemaining > 0` 时前端提供「登记客户退款」跳转 `/fi/payments?arCreditMemoId=`。有有效客户退款时禁止 VOID 贷项。
...
| 贷项 POSTED 且 `refundRemaining > 0` | 允许登记供应商/客户退款 | 贷项「登记供应商退款」/「登记客户退款」 |
| 贷项有有效供应商/客户退款 | 禁止 VOID 贷项 | 贷项「作废」隐藏 |
| 目标 AP/AR 有 `refundable>0` 的未作废贷项 | 禁止撤原收付款 / VOID 原收付款 | 原收付款「作废」/ 删核销行失败 |
| Payment POSTED void | 冲销凭证 + 回滚核销 | Payment「作废」可用 |
| AUTO 凭证 | 不可手工 edit/void | Journal 仅「查看 + 跳转来源」（含 `SUPPLIER_REFUND` / `SUPPLIER_REFUND_ALLOC` / `CUSTOMER_REFUND` / `CUSTOMER_REFUND_ALLOC`） |
...
正确撤销顺序（已收 AR 已开待退款贷项并已退款）：

```
1. void 客户退款
2. 冲销客户退货（若有）
3. void 应收贷项
4. void 原收款（此时 refundable 已为 0）
5. void 原 AR（若需要）
```
...
| `validation.ar_invoice.void.has_credit_memo` | AR 作废但存在未作废应收贷项 |
...
| `validation.fiscal_period.close.unposted_ar_credit_memos` | 关帐阻塞：未过账应收贷项 |
| `validation.fiscal_period.close.unposted_payments` | 关帐阻塞：未过账 Payment |

---

*文档版本：与后端 `lychee-erp-fi` 模块当前实现同步（含 Payment 先资金后核销、供应商退款 `SUPPLIER_REFUND`、客户退款 `CUSTOMER_REFUND`、`/allocations` API、AR/AP 核销对称校验）。*
