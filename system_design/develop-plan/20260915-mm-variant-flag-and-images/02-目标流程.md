...
2. **本款色抽屉（方案 A，交互收敛在同一抽屉）**：按色上传/删/设主图，无 SKU 也可。权限 `product-models:update`。抽屉**顶部常驻**「本款样图」面板：
   - 分色：提示「每色在下行维护」；顶部面板可隐藏或只读说明。
   - 不分色（`product_model_colors` 为空）：顶部面板直接挂载 `colorId = null` 的图片管理组件（全款共享）。用户不必另找列表「款图」按钮（方案 B 不做）。
3. **delete / set-primary 属主**：
   - 物料：服务签名改为 `deleteImage(materialId, imageId)`；`image.materialId` 不匹配 → 404。`setPrimaryImage` 现网已收 `materialId`，继续校验目标行属于该物料。
   - 款色：`deleteImage(modelId, imageId)` / `setPrimaryImage(modelId, imageId)`（色键从行上的 `colorId` 读取，或与 query 一致）；`image.productModelId` 不匹配 → 404。
4. **款色设主图顺序**（部分唯一索引）：先 `clearPrimary(modelId, colorId)`（含 NULL 键）并 Flush，再 `target.setIsPrimary(true)` + `save`。禁止查出列表后 `saveAll` 同时改多行。
