...
**金额清尾**（对标采购退货最后一行剩余金额）：当本贷项行是「最后一笔把该 AR 行数量贷完」时，充要条件是：

```text
Σ(已过账未作废贷项行.quantity) + 本行.quantity = 原 AR 行.quantity
```

不要用 `quantity = creditableQty`：`creditableQty` 已扣其他草稿，单独成立时未必贷完该行。

```text
line.subtotal = 原行.subtotal − Σ(已过账未作废贷项行.subtotal)
line.tax      = tax_amount_overridden ? 覆盖值 : 原行.tax − Σ(已过账未作废贷项行.tax)
line.total    = line.subtotal + line.tax
```

表头金额由行汇总。过账仍卡 `total ≤ total − credited`。禁止用 `max(0, …)` 吃掉尾差；对不齐则拒绝过账。
...
- 贷项 VOID：回滚上述两列（不得负）
- 收款 `applyAr` / `restoreAr` **必须**改用上式，禁止再写 `remaining = total − received`。现网 `restoreAr` 的 `.max(ZERO)` **删掉**：减后为负或公式对不齐一律 `ValidationException`，禁止静默截断
...
收款折扣与其它贷项预占：待退款不得超过该票剩余实收现金。对标应付贷项 / 供应商退款 V1，**必须扣 `cashReserved`**：

```text
cashReceived = Σ(
  POSTED STANDARD RECEIPT
  AND allocation_type = INVOICE
  AND ar_invoice_id = 原 AR
  AND allocated_amount          // 不含 discount_amount
)

cashReserved = Σ(
  同一原 AR 上其他 POSTED、非 VOIDED 贷项的 refundable_amount
)

cashAvailable     = cashReceived − cashReserved
refundableGross   = creditMemo.total − appliedAmount

refundableGross = 0                 → 通过
refundableGross ≤ cashAvailable     → 通过
refundableGross > cashAvailable     → 拒绝（禁止静默截断）
```

V1 **不含** `PREPAYMENT` 核销行（与现网 `sumPostedAllocatedByApInvoiceId` 只计 `INVOICE` 对齐）。预收款核到发票后若要把那笔现金退回，本波会因现金上限拒绝，不做预收款退回。
...
| 作废原 AR | 存在非 VOIDED 贷项 → 禁止 VOID 发票。无贷项时 VOID 回吐占用是负 delta，不撞 `issued` 上限；交货行已有 POSTED 客户退货也不拦 VOID（占用本来就能降） |
| 过账收款 | 不直接拦贷项；待退款不得超过 `cashAvailable`（含 `cashReserved`）。有 `refundable>0` 的未作废贷项时禁止撤原收款 |
...
可退款查询 **本波交付**（客户退款波只消费，不再争论归属）：

```text
POST /api/v1/fi/ar-credit-memos/refundable/page
```

固定 `POSTED && refund_remaining > 0`。权限：`/fi/ar-credit-memos:read`。
...
| 关帐未过账贷项 | `FiscalPeriodManageServiceImpl`（按 `credit_date`）。客户退款复用现网 `payments` 关帐检查，本专题不另加 |
| 回减占用 | 现有 `RemoteDeliveryService.updateInvoicedQuantity`（加交货行锁） |
| VOID 时是否有已过账退货 | `RemoteCustomerReturnService.existsPostedByDeliveryItemIds`（接口本波先加；客户退货未落地前 stub `false`） |
| 可退款列表 | 本波 `refundable/page` |
| 客户退货可退 | **不改**计算器；CreditMemo V1 断言由客户退货波落地 |
...
- 纯金额 / 改单价的价差贷项
- 账龄报表
- 客户预收款退回（不是本贷项路径）。`PREPAYMENT` 核销不计入 `cashReceived`，因此仅靠预收核销的已收票无法形成待退款
